...
class CheckinsController extends AppController
{
    ...
    public function index()
    {
        ...
        $checkins->select(['id', 'checkin_datetime', 'checkout_datetime', 'user_id', 'company_id' => 'inductions.company_id',
            'company_name' => 'companies.name', 'fname' => 'users.first_name', 'lname' => 'users.last_name','full_name'=>'concat(users.first_name,users.last_name)',
            'role' => 'users.role',
        ])->join([
            "table" => "users",
            "type" => "LEFT",
        ...
        if ($type == 0){
            ...
            $key = str_replace(' ', '', $key);
            if ($key) {$checkins->find('all')->where(['concat(users.first_name, users.last_name) like' => '%' . $key . '%'] );}
        }
    ...
    public function checkin()
    {
        ...
        $checkins->select(['id', 'checkin_datetime', 'checkout_datetime', 'user_id', 'company_id' => 'inductions.company_id',
            ...
            'Checkins.project_id' => $project->id,
            'Checkins.checkout_datetime IS' => NULL
        ])->enableAutoFields();
    ...
    public function reminders($id = null)
    {
        ...
        foreach($checkInList as $recipient) {
            ...
            if($currentDateTime > $checkoutLimit) {
